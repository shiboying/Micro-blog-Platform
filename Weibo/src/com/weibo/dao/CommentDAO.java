...
public class CommentDAO extends HibernateDaoSupport {
	...
	public List findByWeiboid(int weiboId){
		log.debug("finding Comment instance by weiboID");
		try{
			String queryString = "from Comment as c where c.weibo.wid="+weiboId;
			return getHibernateTemplate().getSessionFactory().openSession().createQuery(queryString).list();
		}catch (RuntimeException re) {
			log.error("find by Weiboid failed", re);
			throw re;
		}
	}
